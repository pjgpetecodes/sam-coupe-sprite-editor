 ...
 */
function generateInterleavedData(useAmpersand, interleaveMaskFirst) {
    const cells = document.querySelectorAll('.cell');
    let bytes = '';
    for (let i = 0; i < cells.length; i += 2) {

        //
        // Generate the Mask Bytes
        //
        var highMaskNibble;
        if (cells[i].dataset.isNotSprite == 'true') {
            highMaskNibble = 'F';
        ...
        //
        // Generate the Sprite Bytes
        //
        var highSpriteNibble;
        var lowSpriteNibble;
        ...
        {
            highSpriteNibble = colors.indexOf(RgbToHex(cells[i].style.backgroundColor)).toString(16).toUpperCase();
        }
        ...
        {
            lowSpriteNibble = colors.indexOf(RgbToHex(cells[i+1].style.backgroundColor)).toString(16).toUpperCase();
        }
        ...
        //
        // Add the Sprite and Mask Bytes to the Generated Bytes 
        //
        if (interleaveMaskFirst == true)
        {
            bytes += maskByte;
            bytes += spriteByte;    
        }
 ...
 * @param {*} maskData 
 */
function generateData(useAmpersand, interleaveMaskFirst) {
    ...
    var interleavedOutput = generateInterleavedData(useAmpersand, interleaveMaskFirst);
